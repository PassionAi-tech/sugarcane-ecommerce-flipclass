...
import { ArrowDown, ArrowRight, Check, Factory, Leaf, Menu, X, Zap } from 'lucide-react';
import './styles.css';
...
const steps = [
 ...
 {n:'03',title:'Clarification',tag:'CLEAN THE JUICE',text:'The raw juice is heated and treated so suspended impurities can be removed.',visual:'clarify',icon:Factory,fact:'Cleaner juice gives better downstream processing.'},
 {n:'04',title:'Evaporation',tag:'REMOVE WATER',text:'Heat removes water from the clarified juice, turning it into a concentrated syrup.',visual:'evaporate',icon:Zap,fact:'Less water means a much thicker sugar solution.'},
...
function Visual({type}){
 const cubes = [1,2,3,4,5,6].map(i => <i className={`cube-piece p${i}`} key={i}/>);
 return <div className={`process-visual visual-${type}`} aria-hidden="true">
   <div className="scene-grid"/>
   {type==='harvest' && <div className="scene-3d"><div className="field-3d"/><div className="cane-3d"><i/><i/><i/><i/></div><div className="harvester-3d"><b/><span/><i/></div><div className="dust-3d"/></div>}
   {type==='crush' && <div className="scene-3d"><div className="mill-3d"><div className="roller r1"/><div className="roller r2"/><div className="roller r3"/><div className="roller r4"/></div><div className="cane-log"/><div className="juice-3d"/><div className="bagasse-3d"/></div>}
   {type==='clarify' && <div className="scene-3d"><div className="tank-3d"><div className="tank-liquid"/><div className="tank-rim"/></div><div className="filter-3d"><span/><span/><span/></div><div className="pipe-3d"/></div>}
   {type==='evaporate' && <div className="scene-3d"><div className="pan-3d"><div className="syrup-3d"/></div><div className="steam-3d"><i/><i/><i/></div><div className="flame-3d"><i/><i/><i/></div></div>}
   {type==='crystal' && <div className="scene-3d crystal-scene"><div className="crystal-core-3d"/>{cubes}</div>}
   {type==='finish' && <div className="scene-3d"><div className="centrifuge-3d"><div className="centrifuge-inner"/><span>SPIN</span></div><div className="crystal-rain">✦ ✦ ✦ ✦</div><div className="molasses-3d"/></div>}
 </div>
}
...
function App(){
 ...
 <main>
 <section className="hero" id="home"><div className="hero-copy"><div className="overline"><span/> SUGARCANE PROCESSING / FLIP CLASS 2026</div><h1>From cane<br/><em>to crystal.</em></h1><p>A visual journey through the science of sugarcane processing — from a mature stalk in the field to the sugar crystals we know.</p><button className="hero-cta" onClick={()=>go('process')}>Explore the process <ArrowDown size={16}/></button><div className="hero-note"><span>06 stages</span><b/><span>1 crop</span><b/><span>many products</span></div></div><div className="hero-art"><div className="grid-lines"/><div className="hero-orbit orbit-a"/><div className="hero-orbit orbit-b"/><div className="hero-3d"><div className="cane-cylinder"><i/><i/><i/><i/><i/></div><div className="leaf-3d l1"/><div className="leaf-3d l2"/><div className="leaf-3d l3"/><div className="hero-ring"/></div><div className="hero-chip"><small>3D ORIGIN MODEL</small><strong>SUCROSE<br/>STORED HERE</strong></div><div className="hero-index">SCROLL<br/><ArrowDown size={15}/></div></div></section>
 ...
 <section className="intro"><div className="section-tag">THE JOURNEY</div><h2>One stalk.<br/><em>Six transformations.</em></h2><p>Now shown as animated 3D scenes: mechanical separation, heat and controlled crystallization move plant material into finished sugar.</p></section>
